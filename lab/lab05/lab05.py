...
def add_trees(t1, t2):
    ...
    if is_leaf(t1) and is_leaf(t2):
        ...
        return tree(label(t2)+label(t1),[copy_tree(b) for b in branches(t2)])
    elif is_leaf(t2) and (not is_leaf(t1)):
        return tree(label(t1)+label(t2),[copy_tree(b) for b in branches(t1)])
    else:
...
def build_successors_table(tokens):
    ...
    table = {}
    prev = '.'
    for word in tokens: #we
        if prev not in table: # .不在
            "*** YOUR CODE HERE ***"
            table[prev]=[word] #{.：we}
        else:
