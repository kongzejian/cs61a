...
class Player:
    ...
    def play(self, card_index):
        ...
        """
        "*** YOUR CODE HERE ***"
        paly_the_hand=self.hand[card_index]
        for i in range(card_index,len(self.hand)-1):
            self.hand[i]=self.hand[i+1]
        del self.hand[len(self.hand)-1]
        return paly_the_hand
...
class ProfessorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        0
        """
        orig_opponent_deck_length = len(opponent.deck.cards)#对手卓上的卡牌数
        "*** YOUR CODE HERE ***"
