class VendingMachine:
    ...
    def vend(self):
        if self.count==0:
            ...
            self.count-=1
            display='Here is your {0} and ${1} change.'.format(self.product,self.balance-self.price)
            self.balance = 0
            return display
        else:
            self.count-=1
...
def store_digits(n):
    ...
    if n//10==0:
        ...
        first=n//count
        n=n%count
        return Link(first,store_digits(n))
...
def is_bst(t):
    ...
    if not t.branches:
        ...
        left = t.branches[0]
        right = t.branches[1]
        if is_bst(left) and is_bst(right) and bst_max(left)<=t.label and bst_min(right)>=t.label:
            return t.label>=left.label and t.label<=right.label
        else:
            return False
