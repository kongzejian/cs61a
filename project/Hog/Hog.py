...
def announce_highest(who, last_score=0, running_high=0):
    ...
    # BEGIN PROBLEM 7
    "*** YOUR CODE HERE ***"
    def comment(s0,s1,last_score=last_score,running_high=running_high):
        if who==0:
            if(s0-last_score>running_high):
                print(s0-last_score,"point(s)! The most yet for Player 0")
                running_high=s0-last_score
            return announce_highest(who,last_score=s0,running_high=running_high)
        else:
            if (s1-last_score > running_high):
                print(s1-last_score, "point(s)! The most yet for Player 1")
                running_high = s1 - last_score
            return announce_highest(who, last_score=s1, running_high=running_high)
